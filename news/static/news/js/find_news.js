async function findNews(tag) {
    const response = await fetch(`http://127.0.0.1:8000/news/find_news_by_tag/?tag=${tag}`);
...
window.onload = async () => {
    ...
    const tag = urlParams.get('query');
    const news = await findNews(tag);
    displayNews(news);
};
